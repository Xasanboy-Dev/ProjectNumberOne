  ...
  import axios from "axios";
   let User:any
    ...
    alert(err.message)
         ...
         function hello(name:string,lastname:String,email:String){
          if(name!==undefined && lastname!==undefined && email!==undefined){ 
         axios.post("http://localhost:8080/dashboard",{name,lastname,email})
         .then(res=>{
          if(res.status == 200){
  ...
  <div class="d-flex justify-content-center">
    <div class="bg-primary m-2 rounded" style=" width:22%;height:650px;border:2px solid black;">
      ...
      <ul class="px-5 pt-2 fs-3" style="cursor:pointer;">
        <li on:click={()=>{hello(User[0].name,User[0].lastname,User[0].email)}} class="d-flex gap-2 pt-3"><i class="text-warning bi bi-house"></i>Dashboard</li>
        <li class="d-flex gap-2 pt-3"><i class="bi bi-gear text-warning"></i>Settings</li>
        ...
        <li class="d-flex gap-2 pt-3"><i class="text-warning bi bi bi-box-arrow-left"></i>Log out</li>
      </ul>
    </div>
   ...
   <div class="p-5 fs-3" style="height:650px">
       <div class="d-flex justify-content-center"><h1>Hi! {User[0].name} <b class="bg-light rounded">👋</b></h1></div> <br><br>
       You must fill your other data for chatting with other members of this network!<br>
       For this you must click on <a href="/setting">here</a>
       <div>You can write to the Author of this network:<br>
         <ul>
          ...
          I was born in  <i>Uzbekistan</i>
           ...
           <li>
            My  <i class="bi bi-github"></i>github address is <i><a href="https://github.com/Xasanboy-Dev">Xasanboy-Dev</a></i>
           </li>
        </ul>
      </div>
      </div>

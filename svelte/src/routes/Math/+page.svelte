<script lang="ts">
	...
	function already(max: number) {
		if (max - 1 == maxNum) {
			alert('You have a record!');
		}
	}
	function save(correct: number) {
		if (correctAnwers > maxNum) {
			axios
				.post('/record', { user })
				.then((res) => {
	...
	function update() {
		one = between(1, 1000);
		two = between(1, 1000);
		third = between(1, 1000);
		fourth = between(1, 1000);
		first = between(1, 1000);
		second = between(1, 1000);
		one = between(1, 1000);
		amal = toDo[between(0, 3)];
...
<main>
	{#if PlayerName !== undefined}
		<dv class="d-flex justify-content-start mx-5 mt-2 fs-1" on:click={() => save(correctAnwers)}>
		...
		<div class="container">
			<div class="row m-5 p-5 ">
				<div
					on:click={() => fisrt()}
					class="col-sm  mx-5 border ans  d-flex bg-danger text-light   alijn-items-center justify-content-center"
				>
					{one}
				</div>
				<div
					on:click={() => another()}
					class="col-sm  border mx-5 ans d-flex bg-danger text-light   justify-content-center"
				>
					{two}
				</div>
			</div>
			...
			<div class="gap-5 justify-content-center m-2 d-flex">
				<button class="btn text-light bg-danger" on:click={() => update()}>Refresh</button>
				<a href="/calculator">
